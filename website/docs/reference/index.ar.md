...
## 🎯 عمليات التتبع المدمجة — 151 استعلامًا
...
| 🔑 Identity & Access | 44 | استخدام الجذر · تسجيل الدخول إلى وحدة التحكم/MFA · تصعيد الامتيازات · باب خلفي في سياسة الثقة · إساءة استخدام PassRole · AssumeRole عبر الحسابات · SSO/SAML/OIDC · تعداد بيانات الاعتماد · حذف كيان IAM · الاستيلاء عبر AssumeRoot · إساءة استخدام مجمع مستخدمي/رمز Cognito · إخماد حالات الدعم · تسلسل الأدوار · تتبع بيانات اعتماد الجلسة · استطلاع GetCallerIdentity · تسجيلات دخول اتحادية إلى وحدة التحكم · مجموعات أذونات Identity Center والمسؤول المفوَّض · تسلسل الأدوار · تتبع بيانات اعتماد الجلسة · استدعاءات API بلا مصادقة متعددة العوامل · تسجيلات دخول اتحادية · مجموعات أذونات Identity Center |
...
| 🤖 AI & LLM Abuse | 11 | ارتفاعات استدعاء Bedrock · تفعيل الوصول إلى النماذج · العبث بتسجيل الاستدعاءات · استطلاع بمسح المناطق · دفعات الاستدعاءات الفاشلة · جرد المستدعين/الأصول (LLMjacking) · خزنة رموز AgentCore · تجاوز تفويض البوابة · سلامة الذاكرة · تغيير وضع شبكة بيئة العزل · التلاعب بالمراقبة |
| 🌐 Network & Infrastructure | 16 | SG مفتوحة للإنترنت · حذف سجل تدفق VPC · اختطاف CloudFront · أنفاق VPN/TGW سرية · Elastic IP للقيادة والتحكم · مفاتيح API Gateway · اختطاف Route 53/النطاقات · إضعاف الحماية من هجمات DDoS |
...
<details markdown="1">
<summary>📋 القائمة الكاملة — جميع 151 استعلامًا (انقر للتوسيع)</summary>
...
| 31 | 🔗 Role Chaining (Session → Role) | timeseries | يكتشف قيام جلسة دور مُتحمَّل بتحمّل دور آخر. تبدو استدعاءات AssumeRole المفردة عادية؛ لكن السلسلة هي الطريق الذي يسلكه المهاجم من دور مثيل مخترق إلى الأذونات التي يريدها فعلًا. |
| 32 | 🎫 Session Credential Trace | bar | يلخّص ما فعلته كل جلسة STS مؤقتة (مفتاح وصول ASIA…): عدد الاستدعاءات والخدمات وعناوين IP المصدر والنطاق الزمني. وهو سؤال النطاق الذي يبدأ به كل تحقيق في اختراق بيانات الاعتماد. |
| 33 | 🌐 AssumeRole Target Account (roleArn) | timeseries | يكتشف تجاوز حدود الحسابات بقراءة الحساب الهدف من roleArn المطلوب، وهو ما يعمل حتى عند استيعاب سجلات الحساب المتصل فقط. |
| 34 | 📊 AssumeRole Fan-In by Target Role | bar | يرتّب الأدوار حسب من يتحمّلها ومن أين. الدور الذي يتحمّله عادةً حساب واحد ثم يظهر له فجأة متصل ثانٍ يبرز هنا، بينما تدفنه قائمة الأحداث الخام. |
| 35 | 🔍 GetCallerIdentity Reconnaissance | bar | يعرض استدعاءات GetCallerIdentity لكل كيان وعنوان IP مصدر. إنه أول أمر يُنفَّذ ببيانات اعتماد مسروقة — واستدعاء واحد فقط، لا تبلغه أبدًا عمليات الصيد الاستطلاعية القائمة على عتبات الحجم. |
| 36 | 🪪 Federated Console Logins | timeseries | يسرد عمليات تسجيل الدخول إلى وحدة التحكم الواردة عبر موفّر هوية خارجي، مع اسم الموفّر والمصدر. عندما يكون الموفّر هو المكوّن المخترق، لا ترى AWS سوى تسجيل دخول صالح. |
| 37 | 🎟 Identity Center Permission Set Grants | timeseries | يكتشف إنشاء مجموعات الأذونات وإرفاق السياسات وتعيينات الحسابات في IAM Identity Center — وهو الطريق إلى وصول إداري دائم في كل حساب داخل المؤسسة. |
| 38 | 🧑 Identity Store User & Group Creation | timeseries | يكتشف المستخدمين والمجموعات والعضويات المنشأة مباشرة في مخزن هويات Identity Center — استمرارية لا تظهر في IAM إطلاقًا ومن ثم تفوتها المراقبة المقتصرة على IAM. |
| 39 | 👑 Delegated Administrator Registration | timeseries | يكتشف تسجيل مسؤول مفوَّض لخدمة مؤسسية. وهو الحدث الوحيد الذي يصنّفه دليل Identity Center على أنه CRITICAL: فهو يسلّم التحكم على مستوى المؤسسة إلى حساب آخر. |
...
| 7 | 🔑 AgentCore Token Vault Abuse | bar | يجمّع إصدار خزنة رموز AgentCore حسب الكيان والمصدر. توزّع هذه الاستدعاءات رموز OAuth ومفاتيح API لأطراف ثالثة، ولذلك تمتد إساءة الاستخدام إلى خدمات خارج AWS. |
| 8 | 🚪 AgentCore Gateway Authorization Bypass | timeseries | يكتشف تغييرات بوابة AgentCore وسياساتها، بما في ذلك خفض محرك سياسات Cedar إلى LOG_ONLY. التفويض الذي يكتفي بالتسجيل لا يزال يعيد النجاح، فلا يبدو أي شيء خاطئًا في المراحل التالية. |
| 9 | 🧠 AgentCore Memory Integrity | timeseries | يكتشف تغييرات Memory وRegistry في AgentCore، بما في ذلك إعادة توجيه تدفق الذاكرة إلى ARN لـ Kinesis في حساب آخر. الذاكرة طويلة الأمد الملوثة تبقى في كل جلسة لاحقة للوكيل. |
| 10 | 📦 AgentCore Sandbox Network Mode Drift | timeseries | يسرد أحداث دورة حياة مفسّر الشيفرة والمتصفح في AgentCore مع وضع الشبكة. لا يمكن تعديل الوضع، لذا فإن الحذف ثم إعادة الإنشاء هو السبيل الوحيد لتوسيع وصول بيئة العزل إلى الشبكة. |
| 11 | 🙈 AgentCore Observability Tampering | timeseries | يكتشف تغييرات مقيّمات AgentCore وتغييرات أخذ العينات أو وجهات التتبع في X-Ray. المقيّم الذي ينشئه المهاجم يقرأ كل استجابة يقيّمها، فيصدّر مخرجات النموذج عبر قناة مشروعة. |
...
| 16 | 🛡 DDoS Protection Weakening | timeseries | يكتشف إضعاف حماية الحافة بدلًا من إزالتها: تحويل الإجراء الافتراضي لـ WebACL إلى السماح، وتخفيف مجموعات القواعد، وحذف حماية Shield، وإعادة توجيه أصول CloudFront. |
