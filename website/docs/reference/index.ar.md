...
## 🎯 عمليات التتبع المدمجة — 139 استعلامًا
...
|----------|:-------:|---------------------|
| 🛡 Detection & Response | 14 | العبث بخدمة التدقيق (CloudTrail/GuardDuty/Config/SecurityHub/Macie) · حذف SCP · إخماد الإنذارات · تسريب السجلات · ربط سلسلة هجوم برامج الفدية |
| 🔑 Identity & Access | 36 | استخدام الجذر · تسجيل الدخول إلى وحدة التحكم/MFA · تصعيد الامتيازات · باب خلفي في سياسة الثقة · إساءة استخدام PassRole · AssumeRole عبر الحسابات · SSO/SAML/OIDC · تعداد بيانات الاعتماد · حذف كيان IAM · الاستيلاء عبر AssumeRoot · إساءة استخدام مجمع مستخدمي/رمز Cognito · إخماد حالات الدعم · تسلسل الأدوار · تتبع بيانات اعتماد الجلسة · استطلاع GetCallerIdentity · تسجيلات دخول اتحادية إلى وحدة التحكم · مجموعات أذونات Identity Center والمسؤول المفوَّض · استدعاءات API بلا مصادقة متعددة العوامل |
| 🪣 Data & Storage | 31 | حذف/تنزيل جماعي في S3 · قراءة جماعية للأسرار · العبث بالنسخ الاحتياطية · عمليات KMS · مشاركة اللقطات · تسريب EBS Direct API · تصدير DynamoDB · نسخ S3 عبر الحسابات · تشفير SSE-C لبرامج الفدية · الحذف المُحفَّز بدورة الحياة · التلاعب بـ RDS Data API · إعادة تشفير التخزين للتأثير · وضع رسالة الفدية · تحديد نطاق الإخطار بالاختراق · نسخ الكائنات عبر الحسابات · إنشاء عناوين URL موقَّعة مسبقًا |
...
| 🤖 AI & LLM Abuse | 10 | ارتفاعات استدعاء Bedrock · تفعيل الوصول إلى النماذج · العبث بتسجيل الاستدعاءات · استطلاع بمسح المناطق · دفعات الاستدعاءات الفاشلة · خزنة رموز AgentCore · تجاوز تفويض البوابة · سلامة الذاكرة · تغيير وضع شبكة بيئة العزل · التلاعب بالمراقبة |
| 🌐 Network & Infrastructure | 13 | SG مفتوحة للإنترنت · حذف سجل تدفق VPC · اختطاف CloudFront · أنفاق VPN/TGW سرية · Elastic IP للقيادة والتحكم · مفاتيح API Gateway · اختطاف Route 53/النطاقات · إضعاف الحماية من هجمات DDoS |
| 🕵 Threat Patterns | 11 | دفعة استطلاع · وكلاء مستخدم غير معتادين · انتشار متعدد المناطق · استدعاءات API لأول مرة · نشاط منطقة مشاهَد لأول مرة · نشاط خارج ساعات العمل · تصعيد الامتيازات ذاتيًا · انحراف الحجم اليومي · إنشاء موارد في مناطق غير مستخدمة · استدعاءات API عالية الحجم |
| 📊 Activity & Baseline | 3 | أحداث الكتابة في وحدة التحكم · ارتفاعات الأخطاء · الأخطاء الأخيرة |
| 🌍 GeoIP Analysis | 2 | تسجيلات الدخول/الرفض/الكتابة إلى وحدة التحكم حسب البلد · الوصول من بلد نادر |
| ☁ IaC & Platform | 2 | سلسلة توريد CI/CD · إساءة استخدام CloudFormation |

<details markdown="1">
<summary>📋 القائمة الكاملة — جميع 139 استعلامًا (انقر للتوسيع)</summary>
...
| 11 | 🩺 Security Monitoring Posture Recon | timeseries | يكشف الفحص للقراءة فقط لمنظومة المراقبة نفسها — هل يعمل مسار التتبع، هل GuardDuty مُفعَّل، هل Config يسجّل. إنها الخطوة السابقة للتهرب الدفاعي، وآخر خطوة تترك سجلاً نظيفاً. |
| 12 | 💰 Budget / Cost Anomaly Changes | timeseries | يكتشف حذف أو تعديل ميزانيات AWS ومراقبات شذوذ التكلفة. يزيل المهاجمون تنبيهات الميزانية لإخفاء تعدين العملات المشفرة أو العمليات كثيفة الموارد. |
| 13 | 🚫 Access Denied Errors | bar | يجمّع أخطاء AccessDenied حسب الهوية وAPI. أبرز المخالفين قد يشيرون إلى إساءة استخدام بيانات الاعتماد. |
| 14 | ⛓ Ransomware Kill-Chain Sequence | bar | يربط مراحل برامج الفدية الثلاث — إزالة وسائل الاسترداد، وتعطيل الحماية، وتدمير البيانات أو تشفيرها — لكل كيان ويوم. كل مرحلة بمفردها ضجيج تشغيلي؛ لكن الثلاث معًا ليست كذلك. |
...
| 12 | 🏢 Cross-Account Access | timeseries | يجد الأحداث حيث يختلف حساب المستدعي عن حساب المستلم. إشارة على الحركة الجانبية. |
| 13 | 🔑 STS Federation Token Issuance | timeseries | يكتشف استدعاءات GetFederationToken وGetSessionToken. يستخدمها المهاجمون لتحويل المفاتيح طويلة الأمد إلى بيانات اعتماد مؤقتة مستمرة. |
| 14 | 🧩 STS AssumeRoleWithWebIdentity | timeseries | يكتشف استدعاءات AssumeRoleWithWebIdentity. إساءة استخدام ثقة OIDC خاطئة التهيئة (مثل مطالبة sub واسعة جدًا) تتيح للمهاجمين اختطاف دور باستخدام رموز يتحكمون فيها. |
| 15 | 🆔 IAM Identity Center (SSO) Events | timeseries | يكتشف إجراءات إدارة AWS IAM Identity Center. يسيء المهاجمون استخدام SSO لإنشاء مجموعات أذونات خلفية أو تعيين حسابات لمستخدمين يتحكمون فيهم. |
| 16 | 🔗 SAML / OIDC Provider Updates | timeseries | يكتشف تغييرات موفّر هوية SAML/OIDC. تحديث موفّر SAML ببيانات وصفية يتحكم فيها المهاجم يُنشئ بابًا خلفيًا مستمرًا للمصادقة. |
| 17 | 🧐 IAM Access Analyzer Calls | timeseries | يكتشف أي استخدام لـ IAM Access Analyzer. يستخدم المهاجمون المحلل الأصلي من AWS لتعداد الموارد المتاحة خارجيًا دون كتابة نصوص استطلاع مخصصة. |
| 18 | 🔄 Credential Report & Enumeration | timeseries | يكشف تعداد IAM الذي يرسم من هو موجود وما الذي يُسمح له به. تدفّق هذه الاستدعاءات، خاصةً مع AccessDenied، هو المرحلة المبكرة من الهجوم. |
| 19 | 🗝 Access Key Abuse | bar | يكتشف مفاتيح الوصول المستخدَمة من 3 عناوين IP مصدرية متمايزة أو أكثر خلال 7 أيام. مؤشر قوي على تسريب المفتاح. |
| 20 | 📰 AWS Organizations Account Creation | timeseries | يكتشف إنشاء حسابات Organizations وتغييرات المسؤول المفوَّض. ينشئ المهاجمون حسابات ظل لترسيخ مواطئ قدم مستمرة خارج الحساب الرئيسي. |
| 21 | 👥 Cognito Unauthenticated Access | timeseries | يكتشف مجمعات هوية Cognito التي يكون فيها الوصول غير المصادَق عليه مفعّلًا. يتيح للمستخدمين المجهولين استدعاء واجهات AWS بأذونات دور IAM غير المصادَق. |
| 22 | 🧪 Glue DevEndpoint Privilege Escalation | timeseries | يكتشف إنشاء نقطة نهاية تطوير Glue وتعداد الاتصالات. iam:PassRole + glue:CreateDevEndpoint يمنح أذونات الدور كاملة عبر SSH — إحدى أكثر تقنيات تصعيد امتيازات IAM إغفالًا. |
| 23 | 🧪 SageMaker Notebook Privilege Escalation | timeseries | يكتشف إنشاء مثيل دفتر SageMaker وتوليد عنوان URL مُوقَّع مسبقًا. iam:PassRole + sagemaker:CreateNotebookInstance يوفّر بيئة Jupyter بكامل أذونات AWS للدور المُمرَّر. CreatePresignedNotebookInstanceUrl وحدها يمكن أن تمنح الوصول إلى دفتر موجود. |
| 24 | 🪓 IAM Entity Deletion | timeseries | يكتشف حذف مستخدمي IAM وأدواره وسياساته وأجهزة MFA. يحذف المهاجمون كيانات IAM لإزالة آثار نشاطهم أو لإقصاء المدافعين. |
| 25 | 👑 AssumeRoot Usage | timeseries | يكتشف استدعاءات sts:AssumeRoot من الحساب الإداري إلى جذر حساب عضو. يمكن لحساب إداري مخترق الاستيلاء على كل حساب عضو بهذه الطريقة. |
| 26 | 🎫 Support Case Manipulation | timeseries | يكتشف إغلاق حالات AWS Support والتعليق عليها. يحلّ المهاجمون حالات الإساءة/الدعم لإخماد إشعارات AWS بشأن الاختراق. |
| 27 | 🪪 Cognito User Pool Manipulation | timeseries | يكتشف تغييرات مجمع مستخدمي Cognito وعميل التطبيق: تمديد صلاحية الرمز، وعملاء جدد، وإنشاء مستخدم مسؤول. يسيء المهاجمون استخدام هذه لإصدار رموز طويلة الأمد أو زرع مستخدمين خلفيين. |
| 28 | 🔗 Role Chaining (Session → Role) | timeseries | يكتشف قيام جلسة دور مُتحمَّل بتحمّل دور آخر. تبدو استدعاءات AssumeRole المفردة عادية؛ لكن السلسلة هي الطريق الذي يسلكه المهاجم من دور مثيل مخترق إلى الأذونات التي يريدها فعلًا. |
| 29 | 🎫 Session Credential Trace | bar | يلخّص ما فعلته كل جلسة STS مؤقتة (مفتاح وصول ASIA…): عدد الاستدعاءات والخدمات وعناوين IP المصدر والنطاق الزمني. وهو سؤال النطاق الذي يبدأ به كل تحقيق في اختراق بيانات الاعتماد. |
| 30 | 🌐 AssumeRole Target Account (roleArn) | timeseries | يكتشف تجاوز حدود الحسابات بقراءة الحساب الهدف من roleArn المطلوب، وهو ما يعمل حتى عند استيعاب سجلات الحساب المتصل فقط. |
| 31 | 📊 AssumeRole Fan-In by Target Role | bar | يرتّب الأدوار حسب من يتحمّلها ومن أين. الدور الذي يتحمّله عادةً حساب واحد ثم يظهر له فجأة متصل ثانٍ يبرز هنا، بينما تدفنه قائمة الأحداث الخام. |
| 32 | 🔍 GetCallerIdentity Reconnaissance | bar | يعرض استدعاءات GetCallerIdentity لكل كيان وعنوان IP مصدر. إنه أول أمر يُنفَّذ ببيانات اعتماد مسروقة — واستدعاء واحد فقط، لا تبلغه أبدًا عمليات الصيد الاستطلاعية القائمة على عتبات الحجم. |
| 33 | 🪪 Federated Console Logins | timeseries | يسرد عمليات تسجيل الدخول إلى وحدة التحكم الواردة عبر موفّر هوية خارجي، مع اسم الموفّر والمصدر. عندما يكون الموفّر هو المكوّن المخترق، لا ترى AWS سوى تسجيل دخول صالح. |
| 34 | 🎟 Identity Center Permission Set Grants | timeseries | يكتشف إنشاء مجموعات الأذونات وإرفاق السياسات وتعيينات الحسابات في IAM Identity Center — وهو الطريق إلى وصول إداري دائم في كل حساب داخل المؤسسة. |
| 35 | 🧑 Identity Store User & Group Creation | timeseries | يكتشف المستخدمين والمجموعات والعضويات المنشأة مباشرة في مخزن هويات Identity Center — استمرارية لا تظهر في IAM إطلاقًا ومن ثم تفوتها المراقبة المقتصرة على IAM. |
| 36 | 👑 Delegated Administrator Registration | timeseries | يكتشف تسجيل مسؤول مفوَّض لخدمة مؤسسية. وهو الحدث الوحيد الذي يصنّفه دليل Identity Center على أنه CRITICAL: فهو يسلّم التحكم على مستوى المؤسسة إلى حساب آخر. |
...
| 4 | 🔓 S3 Public Access Block Disabled | — | يكشف تعطيل حماية الوصول العام في S3 أو حذفها بالكامل. خطر فوري لكشف البيانات. |
...
| 19 | 📨 SES / SNS Sending Quota Abuse | timeseries | يكشف التجهيزات التي تجعل البريد المزعج مربحاً — رفع حدود إنفاق الرسائل النصية، إعادة تفعيل الإرسال عبر SES، استخدام واجهات الإرسال المجمّع. استدعاءات مفردة منخفضة الحجم لا يمكن لعتبة كل ساعة أن تصل إليها. |
| 20 | 📡 SQS / SNS Cross-Account Policy Changes | timeseries | يكتشف تغييرات سياسة قائمة انتظار/موضوع SQS/SNS التي تمنح وصولًا لحسابات خارجية. تُنشئ قناة تسريب صامتة دون إثارة تنبيهات إرسال عالية الحجم. |
| 21 | 📸 EC2 Public Snapshot / AMI Sharing | timeseries | يكتشف لقطات EBS أو صور AMI المشتركة علنًا (group=all). يتيح لأي شخص نسخ صور القرص واستخراج البيانات. |
| 22 | 📧 Data Exfiltration Channels | bar | يكتشف استدعاءات SNS/SQS/SES/S3 PutObject عالية الحجم (50 أو أكثر في الساعة) التي قد تشير إلى التسريب. |
| 23 | 🔐 S3 SSE-C Encryption (Ransomware) | timeseries | يكتشف إعادة تشفير كائنات S3 بمفاتيح SSE-C يوفّرها المهاجم، إضافة إلى تغييرات التشفير الافتراضي للدلو. بدون مفتاح العميل لا يستطيع الضحية فك التشفير — نمط برامج فدية أصيل للسحابة. |
| 24 | ⏳ S3 Lifecycle-Triggered Deletion | timeseries | يكتشف قواعد دورة حياة S3 التي تُنهي صلاحية الكائنات، إضافة إلى حذف تهيئة دورة الحياة. يضبط المهاجمون انتهاء صلاحية قصير لتطهير البيانات بصمت بمرور الوقت دون إصدار استدعاءات DeleteObject. |
| 25 | 🗃 RDS Query & Instance Manipulation | timeseries | يكتشف استعلامات RDS Data API، وإعادة تعيين كلمة مرور المسؤول، واستعادة اللقطات. يقرأ المهاجمون البيانات مباشرة، أو يعيدون تعيين بيانات الاعتماد للوصول، أو يستعيدون اللقطات إلى مثيلات يتحكمون فيها. |
| 26 | 🔎 S3 Bucket Enumeration | bar | يكتشف المستدعين الذين يمسحون بيانات وصفية للدلو والكائنات (10 قراءات List/GetBucket* أو أكثر في ساعة واحدة). خطوة مبكرة شائعة لتحديد موقع البيانات القيّمة قبل التسريب. |
| 27 | 🔑 Storage Re-Encryption for Impact | timeseries | يكتشف لقطات ووحدات تخزين EBS/RDS المُعاد تشفيرها بمفتاح KMS صريح، إضافة إلى تعطيل التشفير الافتراضي لـ EBS. إعادة التشفير بمفتاح يحوزه المهاجم يحتجز البيانات طلبًا للفدية. |
| 28 | 📝 Ransom Note Placement | timeseries | يكتشف استدعاءات PutObject التي يبدو مفتاح الكائن فيها كرسالة فدية. وخلافًا لبقية عمليات صيد برامج الفدية، يؤكد هذا الأثر بدل الإيحاء به — فوجود الرسالة يعني أن الدفع مطلوب بالفعل. |
| 29 | 📐 Data Access Scope (Breach Notification) | bar | يقيس ما قرأه كل كيان يوميًا: الحاويات التي جرى الوصول إليها والعدد التقريبي للكائنات المتمايزة. ينتج رقم «العدد التقريبي للسجلات» الذي تتطلبه المادة 33 من اللائحة العامة لحماية البيانات. |
| 30 | 📤 Cross-Account Object Copy | timeseries | يكتشف الكائنات المنسوخة بين الحاويات، بما في ذلك استدعاءات PutObject التي تحمل ترويسة x-amz-copy-source. تجهيز البيانات في حساب لا تتحكم به لا يترك سوى هذا الأثر. |
| 31 | 🔗 Presigned URL Generation | bar | يحصي إنشاء عناوين URL الموقَّعة مسبقًا لكل كيان. يسلّم العنوان الموقَّع مسبقًا البيانات لأي شخص يحمل الرابط، دون مصادقة إضافية ودون سجل إضافي في CloudTrail. |
...
| 6 | 🔑 AgentCore Token Vault Abuse | bar | يجمّع إصدار خزنة رموز AgentCore حسب الكيان والمصدر. توزّع هذه الاستدعاءات رموز OAuth ومفاتيح API لأطراف ثالثة، ولذلك تمتد إساءة الاستخدام إلى خدمات خارج AWS. |
| 7 | 🚪 AgentCore Gateway Authorization Bypass | timeseries | يكتشف تغييرات بوابة AgentCore وسياساتها، بما في ذلك خفض محرك سياسات Cedar إلى LOG_ONLY. التفويض الذي يكتفي بالتسجيل لا يزال يعيد النجاح، فلا يبدو أي شيء خاطئًا في المراحل التالية. |
| 8 | 🧠 AgentCore Memory Integrity | timeseries | يكتشف تغييرات Memory وRegistry في AgentCore، بما في ذلك إعادة توجيه تدفق الذاكرة إلى ARN لـ Kinesis في حساب آخر. الذاكرة طويلة الأمد الملوثة تبقى في كل جلسة لاحقة للوكيل. |
| 9 | 📦 AgentCore Sandbox Network Mode Drift | timeseries | يسرد أحداث دورة حياة مفسّر الشيفرة والمتصفح في AgentCore مع وضع الشبكة. لا يمكن تعديل الوضع، لذا فإن الحذف ثم إعادة الإنشاء هو السبيل الوحيد لتوسيع وصول بيئة العزل إلى الشبكة. |
| 10 | 🙈 AgentCore Observability Tampering | timeseries | يكتشف تغييرات مقيّمات AgentCore وتغييرات أخذ العينات أو وجهات التتبع في X-Ray. المقيّم الذي ينشئه المهاجم يقرأ كل استجابة يقيّمها، فيصدّر مخرجات النموذج عبر قناة مشروعة. |

### 🌐 Network & Infrastructure

| # | التسمية | المخطط | الوصف |
|---|-------|:-----:|-------------|
| 1 | 🔥 Security Group Modifications | timeseries | يكتشف تغييرات قواعد مجموعة الأمان، ولا سيما القواعد التي تسمح بـ 0.0.0.0/0 على أي منفذ. |
| 2 | 🌊 VPC Flow Log Changes | timeseries | يكتشف حذف سجلات تدفق VPC. إزالة سجلات التدفق تلغي الأدلة على مستوى الشبكة — مؤشر حرج على التهرب من الدفاع. |
| 3 | 🌐 CloudFront Distribution Tampering | timeseries | يكتشف إنشاء توزيع CloudFront وتغييرات الأصل. تعديل الأصول يعيد توجيه حركة مرور CDN إلى خوادم يتحكم فيها المهاجم لاعتراض الوسيط أو جمع البيانات. |
| 4 | 🧱 Network ACL Changes | timeseries | يكتشف إنشاء وحذف واستبدال إدخالات Network ACL. تتجاوز NACLs مجموعات الأمان ويمكن أن تفتح شبكات فرعية كاملة للمهاجمين. |
| 5 | 🛣️ Route Table Changes | timeseries | يكتشف تعديلات جدول التوجيه. يمكن أن تعيد إضافة أو استبدال المسارات توجيه حركة المرور إلى مضيفين يتحكم فيهم المهاجم (اعتراض الوسيط، اختطاف حركة المرور). |
| 6 | 🧱 VPN / Direct Connect / Transit Gateway | timeseries | يكتشف اتصالات VPN الجديدة وDirect Connect ومرفقات Transit Gateway. ينشئ المهاجمون أنفاق شبكة سرية لقنوات قيادة وتحكم أو تسريب بيانات مستمرة. |
| 7 | 📡 Elastic IP Allocation / Association | timeseries | يكتشف تخصيص وربط Elastic IP. يخصّص المهاجمون عنوان IP عامًا ثابتًا لمثيل مخترق لإنشاء بنية تحتية مستقرة للقيادة والتحكم. |
| 8 | 🗝️ EC2 Key Pair Creation | timeseries | يكتشف أحداث CreateKeyPair وImportKeyPair. ينشئ المهاجمون أو يستوردون مفاتيح SSH كآلية استمرارية للحفاظ على الوصول إلى المثيل. |
| 9 | 📡 Network Infrastructure Changes | timeseries | يكتشف تغييرات VPC ومستوى الشبكة التي قد تُنشئ بنية تحتية يتحكم فيها المهاجم. |
| 10 | 🏷 ACM Certificate Operations | timeseries | يكتشف طلبات وحذف شهادات ACM. يستخدم المهاجمون الحسابات المخترقة لإصدار شهادات TLS لنطاقات يتحكمون فيها لبناء بنية تحتية للتصيد. |
| 11 | 🔑 API Gateway Key Creation & Management | timeseries | يكتشف إنشاء مفتاح API Gateway وإدارة REST API. تُنشئ وحدة api_gateway__create_api_keys في Pacu بيانات اعتماد API مستمرة تنجو من تدوير مفاتيح IAM. كما يعدّل المهاجمون مُصرِّحي API لإضعاف ضوابط الوصول. |
| 12 | 🌐 Route 53 & Domain Changes | timeseries | يكتشف تعديلات سجلات DNS، وتغييرات المنطقة المستضافة، وتسجيل/نقل النطاقات. يعيد المهاجمون توجيه حركة المرور، أو يستولون على نطاقات فرعية معلّقة، أو يسجّلون نطاقات مشابهة للتصيد. |
| 13 | 🛡 DDoS Protection Weakening | timeseries | يكتشف إضعاف حماية الحافة بدلًا من إزالتها: تحويل الإجراء الافتراضي لـ WebACL إلى السماح، وتخفيف مجموعات القواعد، وحذف حماية Shield، وإعادة توجيه أصول CloudFront. |
...
| 4 | 🧭 Single-API Multi-Region Fan-Out | bar | يرصد كياناً واحداً يكرر استدعاء واجهة برمجية واحدة عبر مناطق عديدة خلال ساعة. توقيع المسح الآلي، وغير مرئي لعمليات الصيد الإقليمية الأخرى. |
| 5 | 🕵 First-Time API Calls (24h) | — | يجد استدعاءات API المُشاهَدة في الـ 24 ساعة الأخيرة ولكن لم تُشاهَد من قبل قط. قد تشير العمليات الجديدة إلى أدوات المهاجم. |
| 6 | 🗺 First-Seen Region Activity | bar | يجد مناطق AWS التي يقع أول نشاط لها على الإطلاق ضمن آخر 24 ساعة من مجموعة البيانات. العمل في منطقة لم تُستخدم من قبل طريقة كلاسيكية لإخفاء تعدين العملات المشفرة أو التجهيز عن المراقبة المحصورة بالمنطقة. |
| 7 | 🌙 Off-Hours Activity | bar | يجمّع النشاط حسب الكيان وساعة اليوم داخل نافذة قابلة للضبط خارج ساعات العمل. وهو أول مؤشر يذكره دليل التهديد الداخلي، ولا تغطيه أي عملية صيد أخرى. |
| 8 | 🪞 Self-Service Privilege Escalation | timeseries | يكتشف قيام كيان بتعديل أذوناته الخاصة — إذ يتطابق ARN المتصل مع اسم المستخدم أو الدور الهدف. ترى عمليات صيد التصعيد القائمة المنحَ نفسه لكنها تفقد كونه طُبِّق على الذات. |
| 9 | 📈 Principal Daily Volume Deviation | bar | يقارن حجم الاستدعاءات اليومي لكل كيان بمتوسطه الخاص، مع فصل القراءات عن الكتابات. يلتقط التسريب الذي يستخدم واجهات برمجة مسموحًا بها فقط، حيث يكون الشذوذ في الكمية لا في الفعل. |
| 10 | 🗺 Resource Creation Outside Normal Regions | bar | يشير إلى إنشاء الموارد في مناطق يكاد الحساب لا يستخدمها، مع اشتقاق خط الأساس من البيانات بدل تثبيته في الشيفرة. يظهر هنا كل من تعدين العملات والمشاريع الشخصية. |
| 11 | 📞 High-Volume API Calls per Principal | bar | يسرد أزواج الكيان وواجهة البرمجة التي تتجاوز 50 استدعاءً ناجحًا، مع أول استدعاء وآخره. التعداد والاستخراج الجماعي والحذف الجماعي تتشارك جميعها هذا الشكل. |
...
| # | التسمية | المخطط | الوصف |
|---|-------|:-----:|-------------|
| 1 | 🚨 Unusual Country Access | bar | يكتشف استدعاءات API من بلدان غير متوقعة من خلال عرض مجموعات البلد/الهوية النادرة. |
| 2 | 🚫 Access Denied by Country | bar | يجمّع أخطاء رفض الوصول حسب البلد المصدري. قد تشير حالات الرفض المركزة من بلد واحد إلى هجوم. |
...
## 📊 مخططات لوحة المعلومات — 118 مخططًا
...
| 🎯 Threat Detection | 16 | تجميع شامل للتهرب من الدفاع · فجوات التسجيل · العبث بسجل تدفق VPC/Config/EventBridge/WAF · تغييرات SCP/عضوية المؤسسة · اتجاهات الأخطاء والتقييد · نسبة الكتابة/القراءة · بطاقات مؤشرات محفّزات التصعيد P1/P2 |
| 🔑 Identity & Access | 36 | تسجيلات الدخول إلى وحدة التحكم · اتجاه MFA · خريطة حرارية لتسجيل الدخول · تسلسل مصادقة من الفشل إلى النجاح · استخدام الجذر · نشاط/حذف كيان IAM · الجدول الزمني لتصعيد الامتيازات · كيانات أساسية جديدة · SSO · AssumeRole عبر الحسابات · استخدام AssumeRoot |
...
| 🪣 S3 & RDS | 19 | تنزيل/حذف جماعي في S3 · تعطيل الإصدارات/التسجيل · نسخ عبر الحسابات · سياسة/ACL الدلو · التعداد · تهيئة الحماية · حذف Backup vault · حذف مفتاح KMS · مشاركة لقطة RDS / الحذف بدون لقطة · تشفير SSE-C لبرامج الفدية · الحذف المُحفَّز بدورة الحياة · التلاعب باستعلام/مثيل RDS · إعادة تشفير التخزين للتأثير · نطاق الوصول للإخطار بالاختراق · نسخ الكائنات عبر الحسابات · وضع رسائل الفدية |
...
| 🤖 AI / LLM | 6 | اتجاه استدعاءات Bedrock · تغييرات الوصول إلى النماذج والتسجيل · الاستدعاءات الفاشلة · المستدعون حسب الأصل (فرز LLMjacking) · إصدار رموز AgentCore · تغييرات البوابة والسياسات |
| 🌐 Network | 5 | تغييرات مجموعة الأمان · تغييرات NACL/جدول التوجيه · بنية VPC التحتية · تناظر VPC/Transit Gateway · تغييرات Route53 DNS |
| 🕒 Temporal Analysis | 8 | ارتفاعات سرعة الأحداث · إعادة تنشيط الحسابات الخاملة · أول/آخر مشاهدة حسب الهوية/IP/API/مصدر الخدمة · خريطة حرارية للكتابة خارج ساعات العمل · حجم القراءة/الكتابة اليومي لكل كيان |
| 🌍 GeoIP Intelligence | 6 | السفر المستحيل (كيانات أساسية متعددة البلدان) · أبرز البلدان/المدن/ASN · خريطة العالم · event_name × country |

<details markdown="1">
<summary>📋 القائمة الكاملة — جميع 118 مخططًا (انقر للتوسيع)</summary>
...
| 15 | Security Monitoring Posture Recon | يكشف الفحص للقراءة فقط لمنظومة المراقبة نفسها — هل يعمل مسار التتبع، هل GuardDuty مُفعَّل، هل Config يسجّل. إنها الخطوة السابقة للتهرب الدفاعي، وآخر خطوة تترك سجلاً نظيفاً. |
| 16 | Single-API Multi-Region Fan-Out | يرصد كياناً واحداً يكرر استدعاء واجهة برمجية واحدة عبر مناطق عديدة خلال ساعة. توقيع المسح الآلي، وغير مرئي لعمليات الصيد الإقليمية الأخرى. |
...
| 17 | Role Chaining (Session → Role) | قفزات سلسلة الأدوار — جلسة دور مُتحمَّل تتحمّل دورًا آخر. العمق هو الإشارة. يتطلب العمود المُرقّى session_issuer_arn. |
| 18 | Session Credential Trace (ASIA keys) | ما فعلته كل جلسة STS مؤقتة، مفهرسة بمفتاح الوصول ASIA: عدد الاستدعاءات وواجهات البرمجة المتمايزة وعناوين IP المصدر والمناطق والنطاق الزمني. ابدأ بالجلسة الممتدة عبر عدة عناوين IP. |
| 19 | API Calls Without MFA | استدعاءات الكتابة من جلسات غير موثَّقة بالمصادقة متعددة العوامل. بخلاف بطاقة تسجيلات الدخول بلا MFA، تشمل هذه كل استدعاء API لا ConsoleLogin فقط. |
| 20 | Federated Console Logins by Provider & Origin | تسجيلات الدخول إلى وحدة التحكم عبر موفّر هوية خارجي، مع اسم الموفّر والدولة ورقم النظام الذاتي. عندما يكون الموفّر مخترقًا، لا ترى AWS سوى تسجيل دخول صالح. |
| 21 | Identity Center Permission Set Grants | منح امتيازات IAM Identity Center يوميًا حسب اسم الحدث. مجموعة الأذونات على مستوى المؤسسة: تعيين واحد قد يمنح صلاحية المسؤول في حساب لم يُمَس أبدًا. |
...
| 19 | SES / SNS Sending Quota Abuse | يكشف التجهيزات التي تجعل البريد المزعج مربحاً — رفع حدود إنفاق الرسائل النصية، إعادة تفعيل الإرسال عبر SES، استخدام واجهات الإرسال المجمّع. استدعاءات مفردة منخفضة الحجم لا يمكن لعتبة كل ساعة أن تصل إليها. |
...
| 5 | AgentCore Token Issuance (Daily) | إصدار خزنة رموز AgentCore يوميًا حسب العملية. توزّع هذه الاستدعاءات رموز OAuth ومفاتيح API لأطراف ثالثة، فتمتد إساءة الاستخدام إلى خدمات خارج AWS. |
| 6 | AgentCore Gateway & Policy Changes | تغييرات بوابة AgentCore وأهدافها وسياساتها، مع إظهار وضع محرك سياسات Cedar. الانتقال من ENFORCE إلى LOG_ONLY يظل يعيد النجاح، فلا يبدو شيء خاطئًا لاحقًا. |
...
| 7 | Off-Hours Write Activity (Hour x Day) | أعداد أحداث الكتابة كخريطة حرارية لساعة اليوم مقابل يوم الأسبوع بتوقيت JST. خريطة تسجيلات الدخول تغطي ConsoleLogin فقط؛ وهذه تغطي كل استدعاء مُعدِّل. |
| 8 | Principal Daily Volume (Read vs Write) | حجم الاستدعاءات اليومي لكل كيان، مقسومًا إلى قراءات وكتابات. قِس كل كيان بنفسه: عشرة آلاف استدعاء لدور بناء أمر طبيعي، ومئتان من إنسان ليست كذلك. |
