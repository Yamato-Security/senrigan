...
## 🎯 عمليات التتبع المدمجة — 136 استعلامًا
...
|----------|:-------:|---------------------|
| 🛡 Detection & Response | 13 | العبث بخدمة التدقيق (CloudTrail/GuardDuty/Config/SecurityHub/Macie) · حذف SCP · إخماد الإنذارات · تسريب السجلات · ربط سلسلة هجوم برامج الفدية |
...
| 🕵 Threat Patterns | 10 | دفعة استطلاع · وكلاء مستخدم غير معتادين · انتشار متعدد المناطق · استدعاءات API لأول مرة · نشاط منطقة مشاهَد لأول مرة · نشاط خارج ساعات العمل · تصعيد الامتيازات ذاتيًا · انحراف الحجم اليومي · إنشاء موارد في مناطق غير مستخدمة · استدعاءات API عالية الحجم |
...
<details markdown="1">
<summary>📋 القائمة الكاملة — جميع 136 استعلامًا (انقر للتوسيع)</summary>
...
| 13 | ⛓ Ransomware Kill-Chain Sequence | bar | يربط مراحل برامج الفدية الثلاث — إزالة وسائل الاسترداد، وتعطيل الحماية، وتدمير البيانات أو تشفيرها — لكل كيان ويوم. كل مرحلة بمفردها ضجيج تشغيلي؛ لكن الثلاث معًا ليست كذلك. |
...
| 18 | 🔄 Credential Report & Enumeration | timeseries | يكتشف نشاط تعداد IAM الذي يرسم خريطة كاملة لبيئة IAM. شائع في مراحل الهجوم المبكرة. |
...
| 27 | 📝 Ransom Note Placement | timeseries | يكتشف استدعاءات PutObject التي يبدو مفتاح الكائن فيها كرسالة فدية. وخلافًا لبقية عمليات صيد برامج الفدية، يؤكد هذا الأثر بدل الإيحاء به — فوجود الرسالة يعني أن الدفع مطلوب بالفعل. |
| 28 | 📐 Data Access Scope (Breach Notification) | bar | يقيس ما قرأه كل كيان يوميًا: الحاويات التي جرى الوصول إليها والعدد التقريبي للكائنات المتمايزة. ينتج رقم «العدد التقريبي للسجلات» الذي تتطلبه المادة 33 من اللائحة العامة لحماية البيانات. |
| 29 | 📤 Cross-Account Object Copy | timeseries | يكتشف الكائنات المنسوخة بين الحاويات، بما في ذلك استدعاءات PutObject التي تحمل ترويسة x-amz-copy-source. تجهيز البيانات في حساب لا تتحكم به لا يترك سوى هذا الأثر. |
| 30 | 🔗 Presigned URL Generation | bar | يحصي إنشاء عناوين URL الموقَّعة مسبقًا لكل كيان. يسلّم العنوان الموقَّع مسبقًا البيانات لأي شخص يحمل الرابط، دون مصادقة إضافية ودون سجل إضافي في CloudTrail. |
...
| 6 | 🌙 Off-Hours Activity | bar | يجمّع النشاط حسب الكيان وساعة اليوم داخل نافذة قابلة للضبط خارج ساعات العمل. وهو أول مؤشر يذكره دليل التهديد الداخلي، ولا تغطيه أي عملية صيد أخرى. |
| 7 | 🪞 Self-Service Privilege Escalation | timeseries | يكتشف قيام كيان بتعديل أذوناته الخاصة — إذ يتطابق ARN المتصل مع اسم المستخدم أو الدور الهدف. ترى عمليات صيد التصعيد القائمة المنحَ نفسه لكنها تفقد كونه طُبِّق على الذات. |
| 8 | 📈 Principal Daily Volume Deviation | bar | يقارن حجم الاستدعاءات اليومي لكل كيان بمتوسطه الخاص، مع فصل القراءات عن الكتابات. يلتقط التسريب الذي يستخدم واجهات برمجة مسموحًا بها فقط، حيث يكون الشذوذ في الكمية لا في الفعل. |
| 9 | 🗺 Resource Creation Outside Normal Regions | bar | يشير إلى إنشاء الموارد في مناطق يكاد الحساب لا يستخدمها، مع اشتقاق خط الأساس من البيانات بدل تثبيته في الشيفرة. يظهر هنا كل من تعدين العملات والمشاريع الشخصية. |
| 10 | 📞 High-Volume API Calls per Principal | bar | يسرد أزواج الكيان وواجهة البرمجة التي تتجاوز 50 استدعاءً ناجحًا، مع أول استدعاء وآخره. التعداد والاستخراج الجماعي والحذف الجماعي تتشارك جميعها هذا الشكل. |
